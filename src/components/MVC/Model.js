...
class Model extends UsefulFunctions{
	...
	//!Возвращает массив с индексами ячеек для поля игрока и опонента (for Controller)
	cellsCountArray(a){//a - количество кораблей
		 var shipsArray = [];//Обявляем массив
		 for(var i = 0; i <= a; i++){//Заполняем массив
			 shipsArray.push(i)
		 }
		 return shipsArray;//Возвращаем массив с указаным в аргументе количеством ячеек
	}

	//!Возвращает массив с индексами для кораблей (for Controller)
	shipsCountArray(a){//a - количество ячеек коля
		var countArray = [];//Объявляем массив
		for(var i = 0; i < a; i++){//Заполняем его
			countArray.push(i)
		}
